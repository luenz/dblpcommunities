...
def main():
    ...
    while args.target_function >= 0:
        ...
        number_of_solutions = args.number_of_sol_arg
        output = open(output_path + "outputWEdgeDen" + str(testset) +  str(timeslice_thickness) + str(number_of_solutions) + str(target_fun) + str(community_alg) + ".txt", 'w')
        ...
        output_csv = open(output_path + "outputWEdgeDen" + str(testset) +  str(timeslice_thickness) + str(number_of_solutions) + str(target_fun) + str(community_alg) + ".csv", 'w')
        output_csv_core = open(output_path + "outputWEdgeDenCore" + str(testset) +  str(timeslice_thickness) + str(number_of_solutions) + str(target_fun) + str(community_alg) + ".csv", 'w')
        ...
        while number_of_solutions > 0:
                ...
                for j in range(i, end_year + 1 -start_year - timeslice_thickness + 1):
                    ...
                    for idx, n in enumerate(paths[i * (end_year + 1 -start_year - timeslice_thickness - j)  + j]): 
                        ...
                        if path_len == len(paths[i * (end_year + 1 - start_year - timeslice_thickness - j) + j]) - 1:
                            ...
                            average_edge_den = average_shared * average_size * edge_den_accum * log(path_len - 1)
            ...
            json_out = open(output_path + "outputWEdgeDen" + str(testset) +  str(timeslice_thickness) + str(args.number_of_sol_arg) + str(target_fun) + str(community_alg) + str(args.number_of_sol_arg - number_of_solutions) + ".json", 'w')
            colors_out = open(output_path + "outputWEdgeDenColors" + str(testset) +  str(timeslice_thickness) + str(args.number_of_sol_arg) + str(target_fun) + str(community_alg) + str(args.number_of_sol_arg - number_of_solutions) + ".json", 'w')
